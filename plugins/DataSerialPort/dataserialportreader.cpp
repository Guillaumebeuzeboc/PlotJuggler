...
{		
		...
		port = QInputDialog::getText(nullptr, tr(""),
			tr("Enter serial port:"), QLineEdit::Normal, port, &ok);
		running_ &= ok;
		ok = false;
		int baudrate = 115200;
		baudrate = QInputDialog::getInt(nullptr, tr(""),
			tr("Enter baudrate:"), baudrate, 0, 115200, 100, &ok);
		running_ &= ok;
		ok = false;
		namespace_ = QInputDialog::getText(nullptr, tr(""),
			tr("Enter namespace (or leave blank):"), QLineEdit::Normal, "", &ok);
		running_ &= ok;
		if(running_){
...
void DataSerialPortReader::handleReadyRead()
{
	...
	bool start_with_namespace = line.startsWith(namespace_);
	   ...
	   if(line_list.size()>2 && namespace_.compare("") || line_list.size() > 3){
...
void DataSerialPortReader::handleError(QSerialPort::SerialPortError serialPortError)
{
    if (serialPortError == QSerialPort::ReadError) {
		            qDebug() << "An I/O error occurred while reading, the data from port" << serial_port_.portName() << "error: "<<serial_port_.errorString();
    }
}
